...
# 生成带有时间戳的日志文件名
log_filename = datetime.now().strftime('socket_new_connection_%Y%m%d-%H%M%S.log')
...
def raw_socket_new_connection(url, port=443, requests_count=300):
    """
    使用原始 socket 每次请求新建连接
    """
    # 解析目标地址
    ...
    for i in range(requests_count):
        ...
        try:
            ...
            request = (
                ...
                f"Host: {host}\r\n"
                "Connection: close\r\n"  # 确保每次连接都关闭
                "User-Agent: PythonNewConnection/1.0\r\n"
                "\r\n"
            ).encode('utf-8')
...
def run_threads(target_url, port, thread_count, requests_count):
    ...
    for _ in range(thread_count):
        thread = threading.Thread(target=raw_socket_new_connection, args=(target_url, port, requests_count))
        thread.start()
